...
class CrossValidator:
    ...
    def compute(self):
        for i in range(self.partition):
            self.train_data, self.val_X, self.val_Y = self.create_data(i, augment=False)
...
# In[6]:
#loss func
def compute_loss(y,gy,ratio=3):
    b_s=y.shape[0]
    y=y.view(b_s, -1)
    temp=gy.cpu().detach().numpy()
...
validator = CrossValidator(model, data, compute_acc=compute_acc, loss_function=compute_loss, decoder=decode, lr=1e-4, epochs=300, batch_size=100)
...
import pickle
f=open('unet_val_smooth_nonaug.pk', 'wb')

...
# naive LSTM model trained with smooth long data
...
def train(input_tensor, target_tensor, decoder, decoder_optimizer, criterion, verbose = False, penalty = (1,0.5)):
    ...
    if verbose:
        ...
        print("Target:", target_tensor.squeeze().cpu().numpy())
...
def trainIters(decoder, n_iters, print_every = 1000, plot_every = 100, learning_rate = 0.01, total_batch = maximum_target, penalty = (1, 0.5), gamma = 0.1):    
    ...
    for iter in range(1, n_iters + 1):
        ...
        target_tensor = torch.from_numpy(target_Tensor[num]).to(device)
        ...
        if iter % print_every == 0:
            acc, accuracy = validate(decoder, train_X[1300:], train_Y[1300:])
            ...
            if acc>best_val_acc:
                best_model = torch.save(decoder.state_dict(), "lstm_best_model_long.pt")
                best_val_acc = acc
...
trainIters(model, 15000, print_every=100, learning_rate=1e-3, total_batch = 1300, gamma = 0.5)

...
class DecoderRNN(nn.Module):
    def __init__(self, input_size, augmented_size, hidden_size, output_size, dropout_p = 0):
        ...
        self.cnn1 = nn.Conv1d(self.input_size, self.augmented_size//16 , kernel_size=3, padding=1)
        self.relu1 = nn.ReLU()
        self.cnn2 = nn.Conv1d(self.augmented_size//16, self.augmented_size//8, kernel_size=3, padding=1)
        self.relu2 = nn.ReLU()
        self.cnn3 = nn.Conv1d(self.augmented_size//8, self.augmented_size//4 , kernel_size=3, padding=1)
        self.relu3 = nn.ReLU()
        self.cnn4 = nn.Conv1d(self.augmented_size//4, self.augmented_size//2, kernel_size=3, padding=1)
        self.relu4 = nn.ReLU()
        self.cnn5 = nn.Conv1d(self.augmented_size//2, self.augmented_size, kernel_size=5, padding=2)
        self.relu5 = nn.ReLU()
...
cv = CrossValidator(model, partition=5, epochs=8, batch_size = batch_size, augment_data=0, print_every = 100, penalty = [1, 0.5])
...
f = open("/home/yiqin/2018summer_project/saved_model/Bi-LSTM5-CNN5_batch_losses(cv).pkl", "wb")
pickle.dump(dic, f)

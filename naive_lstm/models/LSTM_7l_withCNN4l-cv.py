...
class DecoderRNN(nn.Module):
    def __init__(self, input_size, augmented_size, hidden_size, output_size):
        ...
        self.cnn1 = nn.Conv1d(self.input_size, self.augmented_size//8 , kernel_size=3, padding=1)
        self.relu1 = nn.ReLU()
        self.cnn2 = nn.Conv1d(self.augmented_size//8, self.augmented_size//4, kernel_size=3, padding=1)
        self.relu2 = nn.ReLU()
        self.cnn3 = nn.Conv1d(self.augmented_size//4, self.augmented_size//2, kernel_size=3, padding=1)
        self.relu3 = nn.ReLU()
        self.cnn4 = nn.Conv1d(self.augmented_size//2, self.augmented_size, kernel_size=3, padding=1)
        self.relu4 = nn.ReLU()
    ...
    def compute(self):
        for i in range(self.partition):
            ...
            for j in range(self.epochs):
                ...
                scheduler.step()
                #torch.save(cur_model.state_dict(), '/home/yiqin/2018summer_project/saved_model/Bi-LSTM-CNN4l(cv){}-{}.pt'.format(i,j))
...
f = open("/home/yiqin/2018summer_project/saved_model/Bi-LSTM-CNN4l_losses(cv).pkl", "wb")
pickle.dump(dic, f)

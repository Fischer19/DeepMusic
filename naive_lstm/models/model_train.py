...
def train(input_tensor, target_tensor, decoder, decoder_optimizer, criterion, verbose = False, penalty = (2,1)):
...
def trainIters(decoder, n_iters, print_every = 1000, plot_every = 100, learning_rate = 0.01, total_batch = maximum_target, penalty = (2, 1), gamma = 0.1):    
    start = time.time()
    ...
    for iter in range(1, n_iters + 1):
        ...
        if iter % print_every == 0:
            acc = validate(decoder, train_X[15000:16000], train_Y[15000:16000])
            print_loss_avg = print_loss_total / print_every
            print_loss_total = 0
            print('%s (%d %d%%) %.4f' % (timeSince(start, iter / n_iters),
                                         iter, iter / n_iters * 100, print_loss_avg))
            print("validation accuracy:", acc)
            torch.save(decoder.state_dict(), 'lstm_smooth_train.pt')

...
class BiLSTM_CRF(nn.Module):
    ...
    def _get_lstm_features(self, sequence):
        ...
        lstm_out1=lstm_out1.reshape(SEQ_LEN, self.hidden_dim)
        lstm_in2=F.relu(self.bn1(lstm_out1))
        lstm_in2=lstm_in2.reshape(SEQ_LEN, 1, self.hidden_dim)

        lstm_out2, self.hidden2 = self.lstm2(lstm_in2, self.hidden2)
        lstm_out2=lstm_out2.reshape(SEQ_LEN, self.hidden_dim)
        lstm_in3=F.relu(self.bn2(lstm_out2))
        lstm_in3=lstm_in3.reshape(SEQ_LEN, 1, self.hidden_dim)

        lstm_out3, self.hidden3 = self.lstm3(lstm_in3+lstm_in2, self.hidden3)
        lstm_out3=lstm_out3.reshape(SEQ_LEN, self.hidden_dim)
        lstm_in4=F.relu(self.bn3(lstm_out3))
        lstm_in4=lstm_in4.reshape(SEQ_LEN, 1, self.hidden_dim)

        lstm_out4, self.hidden4 = self.lstm4(lstm_in4+lstm_in3, self.hidden4)
        lstm_out4=lstm_out4.reshape(SEQ_LEN, self.hidden_dim)
        lstm_in5=F.relu(self.bn4(lstm_out4))
        lstm_in5=lstm_in5.reshape(SEQ_LEN, 1, self.hidden_dim)
        
        lstm_out5, self.hidden5 = self.lstm5(lstm_in5+lstm_in4, self.hidden5)
        lstm_out5=lstm_out5.reshape(SEQ_LEN, self.hidden_dim)
        lstm_in6=F.relu(self.bn5(lstm_out5))
        lstm_in6=lstm_in6.reshape(SEQ_LEN, 1, self.hidden_dim)

        lstm_out6, self.hidden6 = self.lstm6(lstm_in6+lstm_in5, self.hidden6)
        lstm_out6=lstm_out6.reshape(SEQ_LEN, self.hidden_dim)
        lstm_in7=F.relu(self.bn6(lstm_out6))
        lstm_in7=lstm_in7.reshape(SEQ_LEN, 1, self.hidden_dim)
    ...
    def neg_log_likelihood(self, sentence, tags):
        ...
        print(forward_score.size())
        print(gold_score.size())
